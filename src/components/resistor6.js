import React from 'react'

const SvgComponent = ({ openModal, selectedColors, ...props }) => (
    <svg style={{
        display: "inline-block",
        position: "absolute",
        top: 0,
        left: 0
    }}
        version="1.1"
        viewBox="0 0 500 500"
        preserveAspectRatio="xMinYMin meet"
        {...props}>
        <title>{'background'}</title>
        <path fill="none" d="M-1-1h506v189H-1z" />
        <g>
            <title>{'Reistor Calculator 6 color bands'}</title>
            <path fill="#666" d="M12.325 81.756h480v26.487h-480z" />
            <path
                d="M120.375 12.112c-12.457 0-21.647 2.337-28.689 6.156-.03.016-.06.038-.09.055-10.466 5.24-15.866 13.238-19.664 21.82-8.415 16.177-12.824 34.444-32.367 40.272-.762.227-1.52.435-2.284.656v26.939c.764.22 1.521.446 2.284.674 37.549 11.197 7.503 68.303 80.81 68.303.426 0 .886-.02 1.34-.036.829.017 1.664.036 2.52.036 17.522 0 59.795-14.936 76.08-14.936h107.433c16.285 0 58.557 14.936 76.08 14.936.856 0 1.69-.02 2.519-.036.455.017.914.036 1.34.036 73.308 0 43.262-57.106 80.81-68.303.763-.228 1.52-.453 2.284-.674V81.07c-.763-.22-1.521-.428-2.283-.655-19.544-5.828-23.952-24.095-32.368-40.272-3.798-8.582-9.197-16.58-19.663-21.82-.03-.017-.06-.038-.09-.055-7.043-3.82-16.233-6.156-28.69-6.156-.419 0-.86.02-1.304.036-.84-.017-1.687-.036-2.555-.036-17.523 0-59.795 14.935-76.08 14.935H200.315c-16.285 0-58.558-14.935-76.08-14.935-.869 0-1.716.02-2.555.036-.445-.017-.886-.036-1.305-.036z"
                fill="#d9bb7a"
            />
            <path
                d="M137.417 16.48v156.124c.145 3.62 4.935.889 7.866 1.014 2.932.125 14.513-3.17 23.853-5.905V21.32c-9.34-2.736-17.564-5.28-24.29-6.155-7.351-1.124-7.515-2.936-7.429 1.315z"
                onClick={openModal(1)}
                fill={selectedColors.band1}
            />
            <path
                d="M180.494 23.766V165.23c6.533-1.902 12.31-3.276 16.199-3.276h15.282V27.043h-15.282c-3.89 0-9.666-1.375-16.199-3.277z"
                onClick={openModal(2)}
                fill={selectedColors.band2}
            />
            <path
                onClick={openModal(3)}
                fill={selectedColors.band3}
                d="M222.407 27.393v134.625h31.323V27.393h-31.323z" />
            <path
                d="M385.155 11.18c-2.961 0-6.82.711-11.182 1.804v162.945c4.362 1.092 8.22 1.803 11.182 1.803.674 0 1.331-.02 1.983-.037.358.018.72.037 1.055.037 6.758 0 12.389-.793 17.13-2.208V14.382c-4.65-2.025-10.253-3.201-17.13-3.201-.33 0-.677.02-1.027.037-.66-.017-1.327-.037-2.01-.037z"
                onClick={openModal(4)}
                fill={selectedColors.band4}
            />
            <path
                d="M332.475 165.729V22.766c-6.533 1.922-12.31 3.311-16.199 3.311h-15.282v136.34h15.282c3.89 0 9.666 1.39 16.2 3.312z"
                onClick={openModal(5)}
                fill={selectedColors.band5}
            />
            <path
                d="M120.717 177.368a56.56 56.56 0 0 0 4.92-.202V11.686a80.54 80.54 0 0 0-4.92-.146c-.34 0-.705.02-1.067.037-.659-.017-1.323-.037-2.005-.037-5.6 0-14.388 2.431-23.728 5.331v155.166c9.34 2.9 18.127 5.33 23.728 5.33.691 0 1.366-.02 2.034-.036.354.017.705.037 1.038.037z"
                onClick={openModal(6)}
                fill={selectedColors.band6}
            />
        </g>
    </svg>
)

export default SvgComponent
